...
namespace CmsWeb.Areas.Public.Models
{
            ...
            if (search != null)
            {
                foreach (var entry in search)
                {
                    if (entry.Value.parse)
                    {
                        ...
                        {
                            sShell = sShell.Replace("sgfchecked=\"" + entry.Key + ":" + value + "\"", "checked=\"checked\"");
                        }
                    }
                    else
                    {
                        sShell = sShell.Replace("sgfchecked=\"" + entry.Key + ":" + entry.Value.values[0] + "\"", "checked=\"checked\"");
                    }
                }
    ...
    public class GroupLookup
    {
        ...
        public const string PATTERN_CLEAN_CHECKED = @"\sgfchecked=""\w*:\w*:\w*""";
